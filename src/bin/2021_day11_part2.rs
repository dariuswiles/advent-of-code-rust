...
//! Challenge part 2
//!
//! Simulates a group of octopuses as they gain energy and flash each cycle. Runs the simulation
//! until all octopuses flashes simultaneously, and returns the number of times the simulation
//! steps until this occurs.
...
impl Grid {
    ...
    /// Steps until all octopuses flash during the same step, and returns the number of steps
    /// performed until this occurs.
    fn step_until_simultaneous_flash(&mut self) -> u32 {
        for step_num in 1..u32::MAX {
            self.simulate_step();
            if self.octopus.iter().flatten().fold(0, |acc, e| acc + *e as u32) == 0 {
                return step_num;
            }
        }
        panic!("A step where all octopuses are synchronized was not found.");
    }
}
...
fn main() {
    ...
    println!("The first simultaneous flash happens immediately after step {}",
        grid.step_until_simultaneous_flash()
    );
}
...
6789998766"#;
    ...
    #[test]
    fn simultaneous_flash () {
        let mut grid = Grid::new(&TEST_INPUT);
        assert_eq!(grid.step_until_simultaneous_flash(), 195);
    }
