...
fn evaluate_packet(p: &Packet) -> u64 {
    if p.packet_type == 4 {
        if let PacketData::Literal(l) = p.data {
            ...
            panic!("Packet type is literal, but data is not literal for packet {:#?}", p);
        }
    }
    ...
    if p.packet_type <= 7 {
        ...
        match p.packet_type {
            0 => { return sub_packet_data.iter().sum(); }
            1 => { return sub_packet_data.iter().product(); }
            2 => { return *sub_packet_data.iter().min().unwrap(); }
            3 => { return *sub_packet_data.iter().max().unwrap(); }
            5 => {  // Greater than
                assert_eq!(sub_packet_data.len(), 2);
                if sub_packet_data[0] > sub_packet_data[1] { return 1; } else { return 0; }
            }
            6 => {  // Less than
                assert_eq!(sub_packet_data.len(), 2);
                if sub_packet_data[0] < sub_packet_data[1] { return 1; } else { return 0; }
            ...
            _ => { panic!("Unrecognized packet type for packet {:#?}", p); }
        }
