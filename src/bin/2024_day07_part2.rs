...
//! test value. The concatenation operator was added in part 2 of the challenge.
...
/// adding and/or concatenating the `subtotal` to all other values in `equation`. Returns the number
/// of ways `test_value` is obtained by trying all combinations of multipyling, adding and
/// concatenating.
fn _check_equation_validity_internal(test_value: u64, equation: &[u64], subtotal: u64) -> u64 {
    ...
    _check_equation_validity_internal(test_value, rest, subtotal * current)
        + _check_equation_validity_internal(test_value, rest, subtotal + current)
        + _check_equation_validity_internal(test_value, rest, concat_integers(subtotal, current))
}

/// Returns an integer formed by treating the inputs as strings and concatenating them into a larger
/// integer, e.g., inputs 123 and 45 result in 12345. `right` must be greater than 0.
///
/// # Panics
///
/// Panics if `right` is 0.
fn concat_integers(left: u64, right: u64) -> u64 {
...
";
    ...
    fn test_check_equation_validity() {
        ...
        assert_eq!(1, check_equation_validity(156, &vec![15, 6]));
        assert_eq!(1, check_equation_validity(7290, &vec![6, 8, 6, 15]));
        assert_eq!(0, check_equation_validity(161011, &vec![16, 10, 13]));
        assert_eq!(1, check_equation_validity(192, &vec![17, 8, 14]));
    ...
    fn test_concat_integers() {
        assert_eq!(12345, concat_integers(123, 45));
        assert_eq!(7123456, concat_integers(7, 123456));
        assert_eq!(991001, concat_integers(99, 1001));
        assert_eq!(99991, concat_integers(9999, 1));
        assert_eq!(7, concat_integers(0, 7));
    }
}
