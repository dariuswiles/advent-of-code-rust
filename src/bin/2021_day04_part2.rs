...
//! is the last card to win and output a challenge answer based on its contents.
...
impl Board {
    ...
    /// Returns the score for a winning board, as per the challenge rules.
...
/// Iterate through all `called_numbers` until all boards win. Select the last board that won
/// and return its score.
///
/// # Panics
///
/// Panics if all called numbers are marked on all boards, but not all boards win.
fn mark_numbers_until_win(called_numbers: Vec<BingoNum>, boards: &mut Vec<Board>) -> u32 {
    for cn in called_numbers {
        let winners = mark_all_boards(boards, cn);

        for w in winners.iter().rev() {
            if boards.len() != 1 {
                boards.remove(*w);
            } else {
                return boards[0].calculate_score(cn);
            }
        }
    }
    panic!("All called numbers have been used, but not all boards have won.");
}
...
fn main() {
    ...
    let answer = mark_numbers_until_win(called_numbers, &mut boards);
