...
//! game and their score. Part 2 adds more complex game rules, including recursion.
...
const MAX_GAME_ROUNDS: u32 = 100000;
...
impl Game {
    ...
    fn play_one_round(&mut self) -> Option<u8> {
        ...
        let winner;
        if (p1_card as usize <= self.player1.len()) && (p2_card as usize <= self.player2.len()) {
            let mut subgame = Game {
                player1: self.player1[..p1_card as usize].to_vec(),
                player2: self.player2[..p2_card as usize].to_vec(),
            };

//             println!("Starting a subgame with following setup: {:?}", &subgame);

            winner = subgame.play_game(MAX_GAME_ROUNDS).expect(
                "Game exited because it got stuck in an infinite loop."
            );
        } else {
            if p1_card > p2_card {
                winner = 1;
...
mod tests {
    ...
    const TEST_INPUT_FOR_INFINITE_LOOP: &str = "\
Player 1:
43
19
...
";
    ...
    fn test_play_game() {
        ...
        let expected = Game {
            player1: vec![],
            player2: vec![7, 5, 6, 2, 4, 1, 10, 8, 9, 3],
        };
    ...
    fn infinite_loop_prevention() {
        ...
        assert_eq!(105, game.score_player(winner.unwrap()));
    }
}
