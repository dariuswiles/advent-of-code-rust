...
//! visible dots. Part 2 of the challenge requires all the folds to be processed and the output
//! displayed so the characters forming the answer can be read.
...
impl fmt::Display for Grid {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let max_x = self.dots.iter().fold(0, |acc, c| max(acc, c.x));
        let max_y = self.dots.iter().fold(0, |acc, c| max(acc, c.y));
