...
//! locations on the grid that are specific distances from every two antennas of the same type.
//! Part 2 specifies that the locations repeat at set intervals from the antennas, increasing the
...
/// pair of antennas. Antinodes are located on each antenna in the pair, and along the line at
/// multiples of the distance between the two antennas.
fn get_antinodes(grid: &Grid) -> HashSet<Position> {
    ...
    for locations in grid.antennas.values() {
        ...
        for ((x_0, y_0), (x_1, y_1)) in location_pairs {
            ...
            let mut x = *x_0;
            let mut y = *y_0;
            while (0..grid.size.0).contains(&x) && (0..grid.size.1).contains(&y) {
...
";
    ...
    fn test_get_antinodes() {
        ...
        assert!(antinodes.contains(&(0, 0)));
        assert!(antinodes.contains(&(0, 1)));
        assert!(antinodes.contains(&(0, 6)));
        assert!(antinodes.contains(&(0, 11)));
        assert!(antinodes.contains(&(1, 1)));
        assert!(antinodes.contains(&(1, 3)));
        assert!(antinodes.contains(&(1, 8)));
        assert!(antinodes.contains(&(2, 2)));
        assert!(antinodes.contains(&(2, 4)));
        assert!(antinodes.contains(&(2, 5)));
        assert!(antinodes.contains(&(2, 10)));
        assert!(antinodes.contains(&(3, 2)));
        assert!(antinodes.contains(&(3, 3)));
        assert!(antinodes.contains(&(3, 7)));
        assert!(antinodes.contains(&(4, 4)));
        assert!(antinodes.contains(&(4, 9)));
        assert!(antinodes.contains(&(5, 1)));
        assert!(antinodes.contains(&(5, 5)));
        assert!(antinodes.contains(&(5, 6)));
        assert!(antinodes.contains(&(5, 11)));
        assert!(antinodes.contains(&(6, 3)));
        assert!(antinodes.contains(&(6, 6)));
        assert!(antinodes.contains(&(7, 0)));
        assert!(antinodes.contains(&(7, 5)));
        assert!(antinodes.contains(&(7, 7)));
        assert!(antinodes.contains(&(8, 2)));
        assert!(antinodes.contains(&(8, 8)));
        assert!(antinodes.contains(&(9, 4)));
        assert!(antinodes.contains(&(9, 9)));
        assert!(antinodes.contains(&(10, 1)));
        assert!(antinodes.contains(&(10, 10)));
        assert!(antinodes.contains(&(11, 3)));
        assert!(antinodes.contains(&(11, 10)));
        assert!(antinodes.contains(&(11, 11)));
    }

    #[test]
    fn test_do_challenge() {
        assert_eq!(34, do_challenge(INPUT));
    }
}
