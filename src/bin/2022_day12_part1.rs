...
/// Returns the shortest path between the `start` and `end` `Position`s in the given `FlatMap`.
//
...
fn find_shortest_path(fm: &FlatMap) -> usize {
    let mut turn = 0;
    ...
    visited[fm.start.y * fm.width + fm.start.x] = Some(0);
    ...
    visited_last_turn.insert(fm.start);
...
";
    ...
    fn test_find_shortest_path() {
        ...
        assert_eq!(find_shortest_path(&fm), 31);
    }
}
